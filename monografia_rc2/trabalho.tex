\chapter{Desenvolvimento do Trabalho}
 ...
 O desenvolvimento deste trabalho consistiu em diversas etapas. Primeiramente, constatou-se a necessidade da implementação de métodos relacionados a propagação de informação, tanto para rodar os experimentos, quanto para fazer a análise dos resultados obtidos, portanto, houve a extensão de um ferramenta voltada para os propósitos estipulados neste trabalho. Com a ferramenta foram feitos diversos experimentos com redes reais e modelos de rede genéricos.

A partir da geração dos experimentos foram feitas validações e análise dos resultados. Em geral, foram feitas comparações entre diferentes \textit{datasets} e diferentes modelos de rede, tanto sob o aspecto de propagação de informação quanto sob o aspecto das métricas da rede, buscando correlação entre ambas. Por fim, houve a simulação de um problema real, de forma que foram apresentadas as aplicações que a simulação de modelos epidemiológicos em redes podem oferecer.

\section{Biblioteca CGraph}
...
\section{Validação da simulação}
...
\section{Propagação em \textit{datasets}}
...
\section{Comparação de propagação entre modelos de rede equivalentes}
